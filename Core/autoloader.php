...
function autoloadByNamespace($className)
{
    ...
    if (file_exists($classPathRoot)) {
        ...
        $excludeClassesArray = array(
            'mongodb'
        );

        $flagThrow = true;

        foreach ($excludeClassesArray as $classExcluded) {
            if(strpos(strtolower($className),$classExcluded)!== false) {
                $flagThrow = false;
            }
        }
        ...
        if($flagThrow) {
            throw new \Exception(
                sprintf(
                    "Class %s can't be loaded file not found(%s,%s,%s)",
                    $className, $classPath, $classPathModules, $rootDirectoryClass
                )
            );
        }
